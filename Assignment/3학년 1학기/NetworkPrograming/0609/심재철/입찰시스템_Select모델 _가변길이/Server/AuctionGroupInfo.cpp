...
_AuctionGroupInfo::_AuctionGroupInfo(_AuctionInfo* info)
{
	auction_info = info;
...
{
	if (user_list->GetCount() == auction_info->Get_UserCount()) return false;
...
{
	if (user_list->GetCount() == auction_info->Get_UserCount()) return true;
	return false;
}
...
{
	if (now_auction_count % 10 == 0) return true;
	return false;
}
...
bool _AuctionGroupInfo::ChangeMaxBidUser()
{
	...
	if (!bid_user->pop(client_info)) return false;

	if (!bid_user->copytop(client_info)) return false;
...
{
	_ClientInfo* client_info;
	if (!bid_user->copytop(client_info)) return nullptr;
	return client_info;
}