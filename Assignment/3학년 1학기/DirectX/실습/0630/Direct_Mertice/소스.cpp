...
VOID SetupMatrices()
{
    ...
    D3DXMATRIXA16 matWorld, matRotation, matTranslation;   
    
    D3DXMatrixIsIdentity(&matWorld);
    D3DXMatrixIsIdentity(&matRotation);
    D3DXMatrixIsIdentity(&matTranslation);
    ...
    D3DXVECTOR3 vPostion(0.f, 0.f, 0.f);

    int m_Start = timeGetTime() % count;

    vPostion.x = m_Start * (2.0f * D3DX_PI) / count;

    D3DXMatrixRotationY(&matRotation, fAngle); 
    D3DXMatrixTranslation(&matTranslation, vPostion.x, vPostion.y, vPostion.z);
