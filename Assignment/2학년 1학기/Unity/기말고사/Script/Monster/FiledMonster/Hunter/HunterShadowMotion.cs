...
public class HunterShadowMotion : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ani.SetFloat("Angle", demon.GetComponent<HunterMotion>().Angle);
    ...
    {
        if (!demon.GetComponent<HunterMotion>().isAtt)
        {
            yield return null;
        }
        else
    ...
    public void Dead()
    {
        if (demon.GetComponent<HunterAI>().death)
        {
