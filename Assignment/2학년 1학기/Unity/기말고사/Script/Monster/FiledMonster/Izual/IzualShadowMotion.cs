...
public class IzualShadowMotion : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ani.SetFloat("Angle", demon.GetComponent<IzualMotion>().Angle);
    ...
    {
        if (!demon.GetComponent<IzualMotion>().isAtt)
        {
            yield return null;
        }
        else
    ...
    public void Dead()
    {
        if (demon.GetComponent<IzualAI>().death)
        {
