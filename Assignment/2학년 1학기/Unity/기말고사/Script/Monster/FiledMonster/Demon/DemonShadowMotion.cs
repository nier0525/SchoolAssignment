...
public class DemonShadowMotion : MonoBehaviour
{
    ...
    void FixedUpdate()
    {
        ani.SetFloat("Angle", demon.GetComponent<DemonMotion>().Angle);
    ...
    {
        if (!demon.GetComponent<DemonMotion>().isAtt)
        {
            yield return null;
        }
        else
    ...
    public void Dead()
    {
        if (demon.GetComponent<DemonAI>().death)
        {
