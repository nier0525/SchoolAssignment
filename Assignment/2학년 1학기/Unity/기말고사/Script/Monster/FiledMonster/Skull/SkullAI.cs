...
public class SkullAI : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        demon_m = GetComponent<SkullMotion>();
        demon_s = GetComponent<SkullStatus>();
    ...
    IEnumerator AttakDelay()
    {
        while (!Exit)
        {
            ...
            if (!death && !player_die)
            {
                Shadow.GetComponent<SkullShadowMotion>().Attak(player_pos);
